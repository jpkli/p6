export default  {
  ...
  readFromArrayBuffer(buf) {
    ...
    let magicWord = this.asciiDecode(buf.slice(0, 6));
    if (magicWord.slice(1, 6) != 'NUMPY') {
      throw new Error('unknown file type ', magicWord);
    }

    let headerLength = this.readUint16LE(buf.slice(8, 10));
    ...
    let data;
    if (typeof this.toJSArrayType[header.descr] !== 'function') {
    ...
    // }
    data = new this.toJSArrayType[header.descr](buf, offsetBytes);
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.onload = () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          let ndarray = this.readFromArrayBuffer(buf);
          resolve(ndarray);
        } else {
